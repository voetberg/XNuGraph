...
class ExplainNetwork(ExplainLocal):
    def __init__(
        ...
        super().__init__(
            data_path, out_path, checkpoint_path, batch_size, test, n_batches
        )
        self.planes = planes
    ...
    def explain(self, data, **kwargs):
        ...
        """
        self.entropy, self.loss = self.explainer.forward(
            data, self.message_passing_steps
        )
    ...
    def visualize(self, explaination=None, file_name="explaination_graph"):
        ...
        figure, subplots = plt.subplots(
            2, len(self.planes), figsize=(len(self.planes) * 6, 2 * 6), sharey="row"
        )
        ...
        for index, plane in enumerate(self.planes):
            loss = [
                ...
                if i not in ["planar_decoded", "nexus_decoded", "output_decoded"]
            ]
            loss_index = [
                i
                for i in self.loss.keys()
            ...
            entropy = [
                self.entropy[i][plane]
                for i in self.entropy.keys()
                if i not in ["planar_to_nexus", "nexus_to_planar", "nexus_to_decoder"]
            ]
            entropy_index = [
                i
                for i in self.entropy.keys()
...
class DynamicExplainNetwork(ExplainNetwork):
    def __init__(
        ...
        self.explainer = DynamicProbedNetwork(
            model=self.model, planes=self.planes, data=self.data
        )

...
class DistributionEntropy: 
    ...
    def __call__(self, distribution1, distribution2) -> Any:
        step1_entropy = torch.distributions.categorical.Categorical(logits=distribution1).entropy().sum()
        step2_entropy = torch.distributions.categorical.Categorical(logits=distribution2).entropy().sum()

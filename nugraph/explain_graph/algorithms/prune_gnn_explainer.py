...
class NonTrainedHeteroGNNExplainer(HeteroGNNExplainer): 
    ...
    def model(self, graph): 
        ...
        class message_passing_model: 
            ...
            def forward(self, x, edge_index_plane, edge_index_nexus, nexus, n_iterations): 
        ...
        x = m.forward(x, edge_index_plane, edge_index_nexus, nexus, n_iterations=self.message_passing_steps)
    ...
    def _train(self, model, graph, node_index=None, loss_history=None, **kwargs):
        ...
        for i in tqdm.tqdm(range(self.epochs)):
            optimizer.zero_grad()
            stepped_graph = get_masked_graph(graph, node_mask=None, edge_mask=self.edge_mask)

...
class NuGraph2(LightningModule):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.encoder = Encoder(in_features, planar_features, planes, semantic_classes)

        self.plane_net = PlaneNet(
            in_features,
            planar_features,
        ...
        if event_head:
            self.event_decoder = EventDecoder(
                planar_features, planes, semantic_classes, event_classes
            )
            self.decoders.append(self.event_decoder)

        if semantic_head:
            self.semantic_decoder = SemanticDecoder(
                planar_features, planes, semantic_classes
            )
        ...
        if vertex_head:
            self.vertex_decoder = VertexDecoder(
                planar_features, vertex_features, planes, semantic_classes
            )
    ...
    def step(self, data):
        ...
        x = self(*self.unpack_batch(batch))
    ...
    def predict_step(self, batch: Batch, batch_idx: int = 0) -> Batch:
        self.step(batch)
        return batch
    ...
    def add_train_args(parser: ArgumentParser) -> ArgumentParser:
        ...
        train.add_argument(
            "--gamma", type=float, default=2, help="Focal loss gamma parameter"
        )
        return parser
